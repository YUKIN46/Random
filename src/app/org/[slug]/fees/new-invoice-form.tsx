...
export default function NewInvoiceForm({
  ...
  return (
    <form onSubmit={submit} className="bg-white border border-neutral-200 rounded-xl p-5 grid grid-cols-3 gap-4">
      <select required value={form.studentId} onChange={(e) => setForm({ ...form, studentId: e.target.value })} className="rounded-lg border border-neutral-300 px-3 py-2 text-sm">
        <option value="">Select student</option>
        {students.map((s) => (
          <option key={s.id} value={s.id}>{s.name}</option>
        ))}
      </select>
      <input required type="number" step="0.01" placeholder="Amount due" value={form.amountDue} onChange={(e) => setForm({ ...form, amountDue: e.target.value })} className="rounded-lg border border-neutral-300 px-3 py-2 text-sm" />
      <input required type="date" value={form.dueDate} onChange={(e) => setForm({ ...form, dueDate: e.target.value })} className="rounded-lg border border-neutral-300 px-3 py-2 text-sm" />
