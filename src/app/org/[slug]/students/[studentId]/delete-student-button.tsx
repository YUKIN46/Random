...
export default function DeleteStudentButton({ slug, studentId }: { slug: string; studentId: string }) {
  ...
  async function del() {
    setLoading(true);
    await fetch(`/api/students/${studentId}?slug=${encodeURIComponent(slug)}`, { method: "DELETE" });
    router.push(`/org/${slug}/students`);
    router.refresh();
  }
  ...
  return (
    <div className="flex items-center gap-3 rounded-md border border-ledger-red/30 bg-ledger-red/5 px-3 py-2">
      <p className="text-xs text-ledger-red">Permanently delete all their records?</p>
      <button
        onClick={del}
        disabled={loading}
